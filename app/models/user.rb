class User < ApplicationRecord
  ...
  validates :first_furigana, format: { with: /\A([ァ-ン]|ー)+\z/}
  validates :family_furigana, format: { with: /\A([ァ-ン]|ー)+\z/}
  ...
  validates :recipient_family_furigana, format: { with: /\A([ァ-ン]|ー)+\z/}
  validates :recipient_first_furigana, format: { with: /\A([ァ-ン]|ー)+\z/}
  validates :zip_cord, format: { with: /\A\d{3}[-]\d{4}\z/}
